...
USE_SUPABASE_AUTH = AUTH_PROVIDER == "supabase"
...
def create_app() -> Flask:
    ...
    def admin_login_post():
        ...
        if "supabase_user_id" in user.keys() and user["supabase_user_id"]:
...
def authenticate_admin_user(email: str, password: str) -> Any | None:
    if USE_SUPABASE_AUTH:
        auth_user = supabase_sign_in(email, password)
        if not auth_user:
            return None
        supabase_user_id = auth_user.get("id", "")
        user = query_one(
            "SELECT * FROM users WHERE supabase_user_id = ? AND status = 'active'",
            [supabase_user_id],
        )
        if not user:
        ...
        if not user["supabase_user_id"]:
            execute("UPDATE users SET supabase_user_id=?, updated_at=? WHERE id=?", [supabase_user_id, utc_now(), user["id"]])
...
def supabase_sign_in(email: str, password: str) -> dict[str, Any] | None:
    if not SUPABASE_URL or not SUPABASE_ANON_KEY:
        raise RuntimeError("AUTH_PROVIDER=supabase requiere SUPABASE_URL y SUPABASE_ANON_KEY.")
...
def supabase_admin_upsert_user(email: str, password: str, name: str, role: str, supabase_user_id: str = "") -> str:
    if not SUPABASE_URL or not SUPABASE_SERVICE_ROLE_KEY:
        raise RuntimeError("Crear usuarios con Supabase Auth requiere SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY.")
    ...
    if supabase_user_id:
        response = supabase_auth_request("PATCH", f"/auth/v1/admin/users/{supabase_user_id}", payload, SUPABASE_SERVICE_ROLE_KEY)
        return response.get("id", supabase_user_id)
    if not password:
        abort(400, "Debes indicar una contraseña inicial para crear el usuario en Supabase.")
    response = supabase_auth_request("POST", "/auth/v1/admin/users", payload, SUPABASE_SERVICE_ROLE_KEY)
    return response.get("id", "")


def supabase_auth_request(method: str, path: str, payload: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Supabase Auth rechazó la solicitud: {body}") from exc
    except URLError as exc:
        raise RuntimeError(f"No se pudo conectar con Supabase Auth: {exc.reason}") from exc
...
def save_user(item: sqlite3.Row | None = None) -> int:
    ...
    supabase_user_id = item["supabase_user_id"] if item and "supabase_user_id" in item.keys() else ""
    if not item and not password:
        abort(400, "Debes indicar una contraseña inicial.")
    if USE_SUPABASE_AUTH:
        supabase_user_id = supabase_admin_upsert_user(email, password, name, role, supabase_user_id or "")
    password_hash = generate_password_hash(password or os.urandom(24).hex())
    if item:
        before = dict(item)
        if password:
            execute(
                "UPDATE users SET name=?, email=?, supabase_user_id=?, password_hash=?, role=?, status=?, updated_at=? WHERE id=?",
                [name, email, supabase_user_id, password_hash, role, status, now, item["id"]],
            )
        else:
            execute(
                "UPDATE users SET name=?, email=?, supabase_user_id=?, role=?, status=?, updated_at=? WHERE id=?",
                [name, email, supabase_user_id, role, status, now, item["id"]],
            )
        audit("update", "users", item["id"], name, changed_fields(before, {"name": name, "email": email, "role": role, "status": status}))
        return int(item["id"])
    record_id = execute(
        """
        INSERT INTO users (name, email, supabase_user_id, password_hash, role, status, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        [name, email, supabase_user_id, password_hash, role, status, now, now],
    )
